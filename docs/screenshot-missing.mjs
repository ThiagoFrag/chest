...
if (!PASS) {
  console.error("set PANEL_PASS env var (e.g. PANEL_PASS=... node docs/screenshot-missing.mjs)");
  process.exit(1);
}
const OUT = process.env.OUT_DIR ?? "./docs/screenshots";
const browser = await chromium.launch({ headless: true });
const ctx = await browser.newContext({ viewport: { width: 1440, height: 900 }, ignoreHTTPSErrors: true, deviceScaleFactor: 2 });
...
const targets = [
  ...
  { path: "/servers/forja-a-3?tab=overview", file: "03b-server-overview-alt.png", waitMs: 3500 }
];
