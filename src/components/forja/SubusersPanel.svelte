<script lang="ts">
  ...
  const PERMISSION_LABELS: Record<string, string> = {
    ...
    edit_config: 'editar properties',
    edit_world: 'resetar/trocar seed',
    manage_backups: 'backups (criar/restaurar)',
    manage_files: 'browser de arquivos',
    manage_players: 'whitelist / ops / bans',
    manage_scheduled: 'tarefas agendadas',
    manage_discord: 'bridge Discord',
    view_logs: 'logs em tempo real',
    delete: 'deletar server (perigoso)'
  };
  ...
  async function remove(id: string, username: string) {
    if (!confirm(`Remover ${username} deste server?`)) return;
    busy = `del-${id}`;
    error = null;
    try {
      const res = await fetch(`/api/servers/${containerName}/subusers`, {
...
<div class="space-y-4">
  <section class="mc-card">
    <header class="flex items-start gap-3 mb-3">
      ...
      <div>
        <h3 class="text-lg" style="text-shadow: 2px 2px 0 #3f3f3f;">SUBUSUÁRIOS DESTE SERVER</h3>
        <p class="text-xs text-white/60 mt-0.5" style="text-shadow: 2px 2px 0 #3f3f3f;">
          dê acesso granular a este server específico. admins do painel já tem tudo automaticamente.
        </p>
      </div>
    </header>
              ...
              <div class="flex-1 min-w-0">
                ...
                {#if editingId !== s.id}
                  <p class="text-[10px] text-white/50 mt-1" style="text-shadow: 2px 2px 0 #3f3f3f;">
                    {s.permissions.length} permissão{s.permissions.length === 1 ? '' : 'ões'}
                  </p>
                {/if}
              </div>
              <div class="flex gap-1">
                {#if editingId === s.id}
                  <button type="button" onclick={() => (editingId = null)} class="mc-btn text-xs">cancelar</button>
                ...
                {:else}
                  <button type="button" onclick={() => startEdit(s)} class="mc-btn text-xs">editar</button>
  ...
  <section class="mc-card">
    <header class="flex items-center gap-2 mb-3">
      ...
      <h3 class="text-lg" style="text-shadow: 2px 2px 0 #3f3f3f;">ADICIONAR SUBUSUÁRIO</h3>
    </header>

    <div class="space-y-3">
      <div>
        <label for="sub-username" class="block text-xs text-white/70 mb-1" style="text-shadow: 2px 2px 0 #3f3f3f;">username (precisa já existir)</label>
        <input
          id="sub-username"
          type="text"
          bind:value={newUsername}
          placeholder="moderador-1"
          class="mc-input"
        />
      </div>

      <div>
        <span class="block text-xs text-white/70 mb-2" style="text-shadow: 2px 2px 0 #3f3f3f;">permissões neste server</span>
